...
(define (compute-length get-line-segment get-start-point get-end-point)
  (let* ([seg (get-line-segment)] [start (get-start-point seg)] [end (get-end-point seg)])
    (sqrt (+ (* (- (car end) (car start)) (- (car end) (car start)))
            (* (- (cdr end) (cdr start)) (- (cdr end) (cdr start)))))))
...
(define (distance x y)
  (let ([get-segment (λ() (cons x y))] [get-start (λ(P) (car P))] [get-end (λ(P) (cdr P))])
...
(define (compute-f-with-step f a b step)
  (let compute-values ([f f][b b][result '()])
    (if ( > a b)
        result
        (compute-values f (- b step) (cons (f b) result)))))
...
(define (compute-perimeter points)
  (let perimeter ([points points]
                  [result (distance (car points) (car (reverse points)))])
    (if (= 1 (length points))
        result
        (perimeter (cdr points) (+ result (distance (car points) (cadr points)))))))
...
; x = prima subsecventa, y = a doua subsecventa, z = (separator + a treia subsecventa)
(define (3-sequence-max numbers separator)
  (let*-values ([(x y) (splitf-at numbers (λ(n) (not (= separator n))))]
                [(y z) (splitf-at (cdr y) (λ(n) (not (= separator n))))])
    (max (sum x) (sum y) (sum (cdr z)))))
