...
%% au_pusca/1
%% au_pusca(-ListaNume)
au_pusca(Lista) :- findall(X, inarmat(X, pusca), Lista).
...
%% arme_bmw/1
%% arme_bmw(-ListaArme)
arme_bmw(ListaArme) :- findall(X, (conduce(Y, bmw), inarmat(Y, X)), ListaArme).
...
arme_marca(Marca, ListaArme) :- bagof(Arma, Nume^suspect(Nume:Marca:Arma), ListaArme).
...
marci_arma_unica(LisraMarci) :- findall(Marca, arme_marca(Marca, [_]), LisraMarci).
...
%
% Prima replică:
% Detectiv A : Știam că nu știi cine-i criminalul. (D1 stie masina, D2 stie arma)
%
...
suspect2(Nume:Marca:Arma) :- suspect1(Nume:Marca:Arma), findall(N, suspect1(N:Marca:_), [_, _ | _]).
...
suspect3(Nume:Marca:Arma) :- suspect2(Nume:Marca:Arma), findall(N, suspect2(N:_:Arma), [_, _ | _]).
...
suspect4(Nume:Marca:Arma) :- suspect3(Nume:Marca:Arma), findall(N, suspect3(N:Marca:_), [_, _ | _]).
...
suspect5(Nume:Marca:Arma) :- suspect4(Nume:Marca:Arma), findall(N, suspect4(N:_:Arma), [_]).
...
suspect6(Nume:Marca:Arma) :- suspect5(Nume:Marca:Arma), findall(N, suspect5(N:Marca:_), [_]).
