...
myReverse([], []).
myReverse([H|T1], Rev) :- myReverse(T1, RevT), myConcat(RevT, [H], Rev).
...
factorial(0, 1).
factorial(N, Fact) :- N > 0, SubN is N - 1, factorial(SubN, AntFact), Fact is N * AntFact.
...
setIntersection(_, [], []).
setIntersection([], _, []).
setIntersection([H1|T1], L2, [H1|T3]) :- member(H1, L2), setIntersection(T1, L2, T3).
setIntersection([H1|T1], L2, L3) :- not(member(H1, L2)), setIntersection(T1, L2, L3).
...
setDiff(L, [], L).
setDiff([], _, []).
setDiff([H1|T1], L2, [H1|T3]) :- not(member(H1, L2)), setDiff(T1, L2, T3).
setDiff([H1|T1], L2, L3) :- member(H1, L2), setDiff(T1, L2, L3). 
...
setUnion([], L, L).
setUnion(L, [], L).
setUnion(L1, L2, L) :- setIntersection(L1, L2, Int), setDiff(L2, Int, Dif), myConcat(L1, Dif, L).
