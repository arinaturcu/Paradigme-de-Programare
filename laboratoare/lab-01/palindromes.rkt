...
(define (palindrome-list? L)
  (cond
    ((null? L) #t)
    ((palindrome? (car L)) (palindrome-list? (cdr L)))
    (else #f)))
...
(define (my-all-pals? n Bases)
  (or (null? Bases)
      (and (palindrome? (num->base n (car Bases))) (my-all-pals? n (cdr Bases)))))

(check-exp-part 'c .5 (my-all-pals? 585 '(2 10)) #t)
(check-exp-part 'd .5 (my-all-pals? 594 '(2 10)) #f)
