...
insertElem (BSTNode val left right) elem
  | val == elem = (BSTNode val left right)
  | val > elem  = BSTNode val (insertElem left elem) right
  | val < elem  = BSTNode val left (insertElem right elem)

findElem :: (Ord a, Eq a) => BST a -> a -> Maybe a
findElem BSTNil _ = Nothing 
findElem (BSTNode val left right) elem
  | val == elem = Just val
  | val < elem = findElem right elem
  | val > elem = findElem left elem

inorder :: BST a -> [a]
inorder BSTNil = []
inorder (BSTNode val left right) = inorder left ++ [val] ++ inorder right 

mapTree :: (a -> b) -> BST a -> BST b
mapTree _ BSTNil = BSTNil
mapTree f (BSTNode val left right) = BSTNode (f val) (mapTree f left) (mapTree f right)


foldlTree :: (b -> a -> b) -> b -> BST a -> b
foldlTree _ acc BSTNil = acc
foldlTree f acc (BSTNode val left right) = foldlTree f (foldlTree f (f acc val) left) right
...
completeBinaryTree x = completeBinaryTreeHelper x Nothing ""

completeBinaryTreeHelper x par func = currentNode
  where
    currentNode = Node x par func leftNode rightNode
    leftNode    = completeBinaryTreeHelper (f x) (Just currentNode) "f"
    rightNode   = completeBinaryTreeHelper (g x) (Just currentNode) "g"
...
extractPath node = case (parent node) of
  Just p  -> (value p, function node) : (extractPath p)
  Nothing -> [] 

stopCond :: (Num a, Ord a) => a -> a -> Bool
stopCond xf val = val > 10 * xf
...
path x0 xf = go nodes
  where
    ...
    go (n : nodes) 
      | value n == xf = reverse (extractPath n)
      | stopCond xf (value n) == True = []
      | otherwise = go nodes
...
consElem elem (Atom x) = List [Atom elem, Atom x]
consElem elem (List list) = List (Atom elem : list)

consList :: SList a -> SList a -> SList a
consList list (Atom x) = List (list : [Atom x])
consList list (List y) = List (list : y)

headSList :: SList a -> SList a
headSList (List x) = head x
headSList _ = error "error head"

tailSList :: SList a -> SList a
tailSList (List x) = List (tail x)
tailSList _ = error "error tail"

deepEqual :: Eq a => SList a -> SList a -> Bool
deepEqual (Atom x) (Atom y) = x == y
deepEqual (List x) (List y) = and $ zipWith deepEqual x y
deepEqual _ _ = False
