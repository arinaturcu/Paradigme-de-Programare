...
-- foldl
reverseList5 :: [a] -> [a]
reverseList5 l = foldl (\b a -> a : b) [] l 

-- foldr
reverseList6 :: [a] -> [a]
reverseList6 l = foldr (\a b -> b ++ [a]) [] l
...
numToBase4 n b = case n of
    0 -> []
    _ -> numToBase2 (quot n b) b ++ [mod n b]
...
computeLength getLineSegment getStartPoint getEndPoint = 
    let (x1, y1) = getStartPoint getLineSegment
        (x2, y2) = getEndPoint getLineSegment
...
computeLength2 getLineSegment getStartPoint getEndPoint = 
    ...
    where 
        (x1, y1) = getStartPoint getLineSegment
        (x2, y2) = getEndPoint getLineSegment
...
newtonRaphsonSolve x_old atol g dg = fst $ head $ dropWhile 
                                                    (\p -> (abs (fst p - snd p)) > atol)
                                                    (zip 
                                                        (newtonRaphson x_old g dg) 
                                                        (newtonRaphson (x_old - g x_old / dg x_old) g dg))
