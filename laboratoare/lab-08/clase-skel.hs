...
emptyBoard = Board[Blank, Blank, Blank, Blank, Blank, Blank, Blank, Blank, Blank]
...
place t p@(i, j) (Board board) = Board $ take n board ++ [cell] ++ drop (n + 1) board
    where
        n = i * 3 + j
        cell = case t of
            'X' -> X
...
instance Show TicTacToe
    where 
        show (Board board) = firstRow ++ "\n" ++ secondRow ++ "\n" ++ thirdRow
            where
                firstRow  = concatMap show $ take 3 board
                secondRow = concatMap show $ take 3 (drop 3 board)
                thirdRow  = concatMap show $ drop 6 board
{-
...
instance Eq TicTacToe
    where
        (==) (Board board1) (Board board2) = board1 == board2
{-
...
instance Ord TicTacToe
    where 
        (<=) (Board board1) (Board board2) = (length $ filter (\elem -> elem == Blank) board1) > (length $ filter (\elem -> elem == Blank) board2)
...
class (Ord a) => PQueue pq a where
    ...
    size :: pq a -> Int
    size queue = length (toList queue)
...
instance (Ord a) => PQueue ListPQ a where
    ...
    top (LPQ []) = Nothing 
    top (LPQ list) = Just $ head list 

    pop (LPQ []) = empty
    pop (LPQ list) = LPQ (tail list)
...
merge n1@(Node r1 val1 left1 right1) n2@(Node r2 val2 left2 right2) 
    | (fst val1) >= (fst val2) = merge2 val1 left1 (merge right1 n2)
    | otherwise  = merge2 val2 left2 (merge right2 n1)
    where
        merge2 val l r 
            | (rank l) < (rank r) = Node ((rank l) + 1) val r l
            | otherwise = Node ((rank r) + 1) val l r


inorder :: LeftistPQ a -> [(Prio, a)]
inorder (Empty _) = []
inorder (Node _ val l r) = inorder l ++ [val] ++ inorder r 
...
instance (Ord a) => PQueue LeftistPQ a where
    ...
    insert val root = merge root (Node 1 val (Empty 0) (Empty 0))

    top (Empty _) = Nothing 
    top node = Just $ nodeVal node 

    pop node = merge (left node) (right node) 
