...
boat(2, 0) :- true.
boat(1, 1) :- true.
% boat(NM, NC) :- NM > 0, NC > 0, NM >= NC, (NM + NC) =< 2.
...
% starea dată.
parseState( state(est, NM, NC), est, NM, NC, NM_Vest, NC_Vest) :- NM_Vest is 3 - NM, NC_Vest is 3 - NC. % de ce nu merge daca scriu NM is 3 - NM_Vest sau NM + NM_Vest is 3
parseState( state(vest, NM, NC), vest, NM_Est, NC_Est, NM, NC) :- NM_Est is 3 - NM, NC_Est is 3 - NC.
...
initial_state(misionari, state(est, 3, 3)) :- true.
...
final_state(misionari, state(vest, 3, 3)) :- true.
...
next_state(misionari, state(Mal1, M1, C1), state(Mal2, M2, C2)) :- opus(Mal1, Mal2), boat(Mb, Cb), Mb =< M1, Cb =< C1, 
    M2 is 3 - M1 + Mb, C2 is 3 - C1 + Cb, 
    M3 is 3 - M2, C3 is 3 - C2,
    safeMisionari(M2, C2), safeMisionari(M1, C1), safeMisionari(M3, C3). 
